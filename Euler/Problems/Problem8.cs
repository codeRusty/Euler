using System;
using System.Collections.Generic;
using System.ComponentModel;
using System.Text;

namespace Euler.Problems
{
    [Description("Largest product in a series")]
    public class Problem8 : AbstractProblem
    {
        public override void Execute()
        {
            Dictionary<string, int> keyValuePairs = new Dictionary<string, int>();
            string input = @"";

            List<int> numbers = new List<int>();

            foreach (char a in input)
            {
                numbers.Add(Convert.ToInt32(a.ToString()));
            }

            List<double> sums = new List<double>();

            int noOfAdjusentDigits = 13;
            int resetIndex = 0;
            for (int index = 0; index <= numbers.Count - 13; index++)
            {
                int sum = 1;
                string multipleNumber = "";
                for (int subIndex = resetIndex; subIndex < noOfAdjusentDigits; subIndex++)
                {
                    if (numbers[subIndex] == 0)
                    {
                        break;
                    }

                    sum = sum * numbers[subIndex];
                    multipleNumber = multipleNumber + numbers[subIndex].ToString();
                    if (subIndex == noOfAdjusentDigits - 1)
                    {
                        keyValuePairs.Add(multipleNumber, sum);
                        sums.Add(sum);
                    }
                }
                resetIndex++;
                noOfAdjusentDigits++;
            }

            Dictionary<string, int> sortedList = new Dictionary<string, int>();

            int largest = 0;
            string largestnums = String.Empty;

            int sortidx = 0;


            foreach (var a in keyValuePairs)
            {
                Console.WriteLine(a.Key);

                if (a.Value > largest)
                {
                    largest = a.Value;
                    largestnums = a.Key;
                }
            }
            Console.WriteLine("laregest : " + largestnums);


        }
    }
}
